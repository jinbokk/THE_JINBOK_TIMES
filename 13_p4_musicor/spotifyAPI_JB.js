    ...
    if (color.classList.contains("colorContainer")) {
      ...
      const getTrackByGenre = async () => {
        ...
        const result = await fetch(
          `https://api.spotify.com/v1/recommendations/?seed_genres=${genreId}&market=US&${searchIndex}&min-popularity=80?limit=30`,
          {
            method: "GET",
            headers: {
              Authorization: "Bearer " + token,
        ...
        likeSongBtn.addEventListener("click", (e) => {
          if (e.target.style.filter == false) {
            e.target.style.filter =
              "invert(15%) sepia(81%) saturate(7389%) hue-rotate(358deg) brightness(102%) contrast(116%)";
          } else {
